...
sub main ($) {
	...
	my $output_list_all    = $cfg->param("planner.outlist_all");
	...
	$logger->debug( "Pdb all list file: ", $output_list_all );
	...
	if(!scalar(@$library_binary_lib) or !scalar(@$library_binary_all)) {
	...
	if (length($output_list_all)) {
		my $string_all = join( "\n", @$library_binary_all );
		write_file( $output_list_all, "$string_all\n" );
	}
